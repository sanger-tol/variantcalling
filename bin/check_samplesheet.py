...
class RowChecker:
        ...
        **kwargs,
        ...
        Args:
            sample_col (str): The name of the column that contains the sample name
                (default "sample").
            first_col (str): The name of the column that contains the datatype (default "datatype").
            second_col (str): The name of the column that contains the CRAM
                ile path (default "datafile").
    ...
    def _validate_first(self, row):
        """Assert that the first entry is non-empty and has the right format."""
        assert len(row[self._first_col]) > 0, "The datatype is required."
        row[self._first_col]

    def _validate_second(self, row):
        """Assert that the CRAM entry has the right format."""
        if len(row[self._second_col]) > 0:
            self._validate_cram_format(row[self._second_col])
    ...
    def validate_unique_samples(self):
        ...
        """
        assert len(self._seen) == len(self.modified), "The sample name and cram must be unique."
